...
class formulario_html {
	...
	function recibir_form($datos_form=""){
		if(isset($_GET["id"])){
			...
			if(strstr($_GET["id"], "tabla:")){
				//contiene información sobre una tabla
				$datos_tabla=$_GET["id"];
				list($nom, $tabla)=explode(":", $datos_tabla);
				if(trim($tabla) != ""){
		...
		if($this->procesar_consulta($datos_consulta, $tipo_campo_sql)){
			return(true);
		}else{
			return(false);
		}	
	...
	function procesar_consulta($datos_consulta="", $tipo_campo_sql=""){
		if($datos_consulta==""){
	...
	function cargar_estructura($tabla="", $campos=""){
...
*/
	...
	function generar($datos) {
		...
		$this->html_form_pre="\n"."<FORM action=\"$action\" method=\"$method\" id_form=\"$_id_formulario\" $onsubmit>".$datos_campos_form;
	...
	function salida($solo_campos=0) {
		if($solo_campos){
			...
			print "Salida del formulario:<BR />".$this->html_form;
		}
	}
